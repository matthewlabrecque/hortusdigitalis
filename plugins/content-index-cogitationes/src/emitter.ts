...
interface Options {
  ...
  // Only pages whose source path starts with this prefix are eligible for the
  // RSS feed. Sitemap/content-index JSON are unaffected. Unset/empty disables
  // the filter (all pages become eligible, matching upstream behavior).
  rssPathPrefix?: string;
}
...
// Only entries whose source file lives under `prefix` are eligible for the RSS
// feed, so the feed only changes when Cogitationes content changes.
function filterByPathPrefix(idx: ContentIndexMap, prefix?: string): ContentIndexMap {
  if (!prefix) return idx;
  const filtered: ContentIndexMap = new Map();
  for (const [slug, content] of idx) {
    if (content.filePath && content.filePath.startsWith(prefix)) {
      filtered.set(slug, content);
    }
  }
  return filtered;
}
...
export const ContentIndex: QuartzEmitterPlugin<Partial<Options>> = (opts) => {
  ...
  const emitAll = async (ctx: BuildCtx, content: ProcessedContent[]): Promise<FilePath[]> => {
    ...
    if (options.enableRSS) {
      const rssIndex = filterByPathPrefix(linkIndex, options.rssPathPrefix);
      outputs.push(
        await write({
          ctx,
